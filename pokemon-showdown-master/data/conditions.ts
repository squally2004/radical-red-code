export const Conditions: { [k: string]: ConditionData } = {
	...
	// New weather "new moon"
	darkness: {
		name: 'Darkness',
		effectType: 'Weather',
		duration: 5,
		durationCallback(source, effect) {
			if (source?.hasItem('Reaper Cloth')) {
				return 999999999999999999999999999999999999999999999999999999;
			}
			return 5;
		},
		...
		onFieldStart(battle, source, effect) {
			if (effect?.effectType === 'Ability') {
				if (this.gen <= 5) this.effectState.duration = 0;
				this.add('-weather', 'New Moon', '[from] ability: ' + effect, '[of] ' + source);
			} else {
	...
	troposphere: {
		...
		onWeatherModifyDamage(damage, attacker, defender, move) {
			if (defender.hasItem('utilityumbrella')) return;
			if (move.type === 'Fire', 'Water', 'Ice', 'Rock', 'Dark') {
				this.debug('Troposphere boost');
				return this.chainModify(1.5);
			}
		},
		onFieldStart(battle, source, effect) {
			if (effect?.effectType === 'Ability') {
				if (this.gen <= 5) this.effectState.duration = 0;
				this.add('-weather', 'Troposphere', '[from] ability: ' + effect, '[of] ' + source);
			} else {
				this.add('-weather', 'Troposphere');
			}
		},
		onImmunity(type, pokemon) {
			if (pokemon.hasItem('utilityumbrella')) return;
		...
		onFieldResidualOrder: 1,
		onFieldResidual() {
			this.add('-weather', 'Troposphere', '[upkeep]');
			this.eachEvent('Weather');
		},
		onFieldEnd() {
			this.add('-weather', 'none');
		...
		onModifySpD(spd, pokemon) {
			if (pokemon.hasType('Rock') && this.field.isWeather('Troposphere')) {
				return this.modify(spd, 1.5);
			}
		},
	},
